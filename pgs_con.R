...
# the rate is Total cases per 100,000 population
# New: PGS & Mapping file
paths$scores_dir <- "scores"                 # Folder containing PGS score files
paths$map        <- "cause_pgs_map.csv"      # Must contain cause_name, pgs_id / score_file are recommended

# If GBD file contains multiple years, specify a target year (otherwise, an average is taken)
target_year <- NA_integer_  # e.g., 2013; set to NA to use all years

# Output
out_dir_fig <- "fig_by_cause_measure_newrate"
...
# Standardize key column names (auto-adapt to different exports)
colmap <- list(
...
if (length(colmap$cause)==0) stop("Cause column (e.g., cause_name / cause) not found in GBD data.")

# ============================ Read 1KG populations & metadata =================
...
# If mapping file doesn't exist, create a template from GBD cause names
if (!file.exists(paths$map)) {
  ...
  stop("Template '", paths$map, "' has been created. Please fill in pgs_id or score_file for each cause and rerun.")
}
...
if (nrow(cause_map) == 0) stop("'", paths$map, "' is empty. Please fill it out before running.")

# ============================ Utility Functions ========================================
# Read a single scores file: identify the first column besides 'sample' as the score
...
# Guess score filename based on pgs_id (tries two common naming conventions)
...
# Common GBD data filtering (age/sex/metric & numeric conversion)
...
# ============================ Analyze Single Cause & Measure =========================
run_one <- function(cname, measure_key = c("prevalence")) {
  measure_key <- match.arg(measure_key)
  
  # 1) Find score file: prioritize score_file, otherwise guess from pgs_id
  row <- cause_map %>% filter(str_to_lower(cause_name) == str_to_lower(cname)) %>% slice(1)
  if (nrow(row) == 0) {
    message("⚠️ Cause not found in mapping file: ", cname)
    return(tibble())
  }
  ...
  if (is.na(sfile) || sfile == "" || !file.exists(sfile)) {
    message("⚠️ PGS score file not found: ", sfile, " (cause: ", cname, ", pgs_id: ", row$pgs_id, ") — Skipped")
    return(tibble())
  }
  
  # 2) Read scores & merge samples, map to country
  ...
  # 4) Calculate mean PGS for each population + join with GBD metric
  ...
  if (nrow(combo_local) == 0) {
    message("⚠️ No data available for plotting (cause: ", cname, ", measure: ", measure_key, ") — Skipping plot")
    return(tibble())
  }
  
  # ⚠️ Key change: Use logit-transformed data for fitting and correlation
  fit <- lm(y_logit ~ mean_pgs, data = combo_local)
  
  r_txt <- sprintf("italic(r) == %.2f", cor(combo_local$mean_pgs, combo_local$y_logit))
  p_txt <- sprintf("italic(p) == %s",
                   if (summary(fit)$coefficients["mean_pgs", "Pr(>|t|)"] < 1e-4) format(summary(fit)$coefficients["mean_pgs", "Pr(>|t|)"], digits=1, scientific=TRUE)
                   else sprintf("%.3f", summary(fit)$coefficients["mean_pgs", "Pr(>|t|)"]))
  
  p <- ggplot(combo_local, aes(x = mean_pgs, y = y_logit, color = super_pop, label = bucket)) +
    geom_point(size = 3) +
    ...
    labs(
      title = paste0(cname, " | ", str_to_title(measure_key), " (Log)"),
      x = "PGS (mean by population)",
      y = paste0(str_to_title(measure_key), " — Log"),
      color = "super_pop"
    ) +
